...
class ExactInference(InferenceModule):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        for p in self.legalPositions:
            if noisyDistance is None:
                allPossible[self.getJailPosition()] = 1.0 # if the ghost is captured
                break
            else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for pos, pr in self.beliefs.items():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for p in self.legalPositions:
            for i in range(num_particles_per_position):
                pos.append(p)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for i in range(len(self.particles)):
                trueDistance = util.manhattanDistance(self.particles[i], pacmanPosition)
                if emissionModel[trueDistance] >= 0:
                    weights.append(emissionModel[trueDistance]) # update beliefs
    ...
    def getBeliefDistribution(self):
        ...
        belief.divideAll(float(self.numParticles))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        ## Alternative approach take
        #
        # n = self.numParticles/len(permutations)
        ...
        for i in range(self.numParticles):
            ...
            particles.append(permutations[index])

        # Reshuffling passes a single case failing in 6
    ...
    def observeState(self, gameState):
        ...
        for i in self.particles:
            for ghost in range(self.numGhosts):
                trueDistance = util.manhattanDistance(i[ghost], pacmanPosition)
                if emissionModels[ghost][trueDistance] >= 0 and noisyDistances[ghost] is not None:
                    w = w * emissionModels[ghost][trueDistance]
            weights.append(w) # update beliefs
            w = 1
            values.append(i)
        ...
        if None in noisyDistances:
            for ghost in range(self.numGhosts):
                ...
                if noisyDistances[ghost] is None:
                    for i in self.particles:
                        ghost_particles.append(self.getParticleWithGhostInJail(i, ghost))
                if len(ghost_particles) > 0:        
                    self.particles = ghost_particles
